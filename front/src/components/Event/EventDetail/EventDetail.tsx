...
const EventDetail: React.FC = observer((): React.ReactElement => {
  ...
  const { loading, data, error } = useQuery(GET_EVENT_BY_ID_AND_OWNER, {
      variables: {
        eventID: id, 
          ...
          <div className='wrap border-x-only relative'>
            <div className='container event-detail-content'>
              ...
              <HeaderEventDetail tab={tab} setTab={setTab} />
              {
                (tab === 1 &&  events[0]) &&
                <>
                  <img src={events[0].image} alt="Ticket" className='mb-3 event-header-detail__img object-cover h-64 w-full object-center' />
                  ...
                  {(typeof userName !== "undefined" || data.events[0].status !== 1) ||
